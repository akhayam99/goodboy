...
work item can belong to: what each class delivers and how each class is
verified. How many slots each category gets is owned by
...
## Org work: the deferred-verification rule
...
- Sourced only from evidence the machine already records: the ledger's
  ...
  false greens from the shared turbo cache), and issues opened after a
