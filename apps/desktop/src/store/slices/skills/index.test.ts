...
describe('store contract', () => {
  ...
  describe('skills', () => {
    it('loadSkills caches per-workspace skill list', async () => {
      ...
      storySpies.invokeSkillList.mockResolvedValueOnce([skill]);
    ...
    it('rescanSkills replaces the cached skill list', async () => {
      ...
      storySpies.invokeSkillRescan.mockResolvedValueOnce([skill]);
    ...
    it('deleteSkill invokes delete and refreshes', async () => {
      const store = useAppStore;
      storySpies.invokeSkillList.mockResolvedValueOnce([]);
      await store.getState().deleteSkill('s1' as SkillId, WS_ID);
      expect(storySpies.invokeSkillDelete).toHaveBeenCalledWith('s1');
