...
describe('workflowTurnBreaker', () => {
  it('grants workflow turns up to the cap', () => {
    expect(claimMany(MAX_UNATTENDED_WORKFLOW_TURNS)).toEqual(
      Array(MAX_UNATTENDED_WORKFLOW_TURNS).fill('granted'),
    );
  });

  it('trips on the first turn past the cap and keeps tripping', () => {
    claimMany(MAX_UNATTENDED_WORKFLOW_TURNS);
  ...
  it('forgets turns older than the window', () => {
    ...
    expect(
      claimWorkflowTurn({
        agentId: AGENT_ID,
        nowMs: T0 + MAX_UNATTENDED_WORKFLOW_TURNS + UNATTENDED_WINDOW_MS,
      }),
    ).toBe('granted');
  });
});
