...
export const fanOutClusters = async (
  ...
  if (scopeEscape !== null) {
    void get().emitNotification(
      ...
      'warning',
      `cluster blocked: ${container.name}`,
      `the plan clusters are not a valid graph: ${scopeEscape}`,
      { sessionId },
    );
    return;
  }
...
export const advanceClusterImplementation = (set: SetFn, get: GetFn) => {
  return async (
    ...
    const completedProgress = hasGraph
      ? pairs.filter((pair) => pair.isCompleted).length
      : children.filter((agent) => isCompletedAmong({ agent, completed: sourceProgress.completed }))
          .length + sourceProgress.released.size;
