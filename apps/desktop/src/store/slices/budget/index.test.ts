...
describe('store contract', () => {
  ...
  describe('budget', () => {
    it('loadBudgetRules replaces budgetRules', async () => {
      ...
      storySpies.invokeBudgetRuleList.mockResolvedValueOnce([rule]);
    ...
    it('saveBudgetRule generates id+createdAt and calls upsert', async () => {
      const store = useAppStore;
      storySpies.invokeBudgetRuleList.mockResolvedValueOnce([]);
      ...
      expect(storySpies.invokeBudgetRuleUpsert).toHaveBeenCalledTimes(1);
      const arg = storySpies.invokeBudgetRuleUpsert.mock.calls[0]?.[0] as BudgetRule;
    ...
    it('saveBudgetRule preserves identity when updating an existing rule', async () => {
      ...
      storySpies.invokeBudgetRuleList.mockResolvedValueOnce([{ ...existing, capUsd: 75 }]);

      await store.getState().saveBudgetRule({ ...existing, capUsd: 75 });

      expect(storySpies.invokeBudgetRuleUpsert).toHaveBeenCalledWith({ ...existing, capUsd: 75 });
      expect(storySpies.invokeBudgetRuleDelete).not.toHaveBeenCalled();
    });
    ...
    it('loadSessionBudget caches the budget keyed by sessionId when DB returns non-null', async () => {
      const store = useAppStore;
      storySpies.invokeSessionBudgetGet.mockResolvedValueOnce({
        sessionId: SESSION_ID,
        softCapUsd: 25,
      });
