...
import { Button, SectionHeader, cn, formatError } from '@goodboy/ui';
...
export const WorkspaceMergeSection = ({ workspaceId }: Props) => {
  ...
  if (candidates.length === 0) {
    return (
      <section id="merge" className="flex flex-col gap-4">
        ...
        />
        <p className="text-sm text-muted-foreground">No other workspaces to group.</p>
      </section>
    );
  }
  ...
  return (
    <section id="merge" className="flex flex-col gap-4">
      <SectionHeader
        label="Group workspaces"
        hint="Move the projects, sessions, and connections of other workspaces into this one, then remove the emptied containers."
      />
