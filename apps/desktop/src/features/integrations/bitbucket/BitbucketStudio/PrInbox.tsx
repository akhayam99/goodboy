...
type Props = {
  ...
  readonly focusedPrId: number | null;
  readonly onSelect: (pullRequest: BitbucketPullRequest) => void;
    ...
    return groups
      .map((group) => ({
        ...
        rows: group.rows.filter(
          (pullRequest) =>
            `#${pullRequest.id}`.includes(normalized) ||
              ...
              <div key={group.key} className="flex flex-col gap-1">
                ...
                <ul className="flex flex-col gap-0.5">
                  {group.rows.map((pullRequest) => {
                    const isActive = pullRequest.id === focusedPrId;
                    return (
