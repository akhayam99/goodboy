...
export const StructuredData = ({ data, depth = 0, label, hasImages = false }: Props) => {
  if (data === null || data === undefined) {
    return <span className="italic text-faint-foreground">null</span>;
  }
  ...
  if (typeof data === 'string') {
    ...
    return <span className="whitespace-pre-wrap break-words text-foreground">{data}</span>;
  }
      ...
      <div className="flex flex-col gap-0.5">
        {data.map((v, i) => (
          <div key={i} className="flex items-start gap-1">
            <span className="shrink-0 text-faint-foreground">{i}:</span>
            <StructuredData data={v} depth={depth + 1} hasImages={hasImages} />
          </div>
        ))}
      </div>
