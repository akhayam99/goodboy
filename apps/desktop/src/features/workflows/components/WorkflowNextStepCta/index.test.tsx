...
describe('pickNextWorkflowStep', () => {
  ...
  it('advances to next step once previous one is completed', () => {
    expect(pickNextWorkflowStep(wf(), preCreated('completed', 'pending', 'pending'))?.id).toBe('s2')
  })
  ...
  it('returns null when all steps are completed', () => {
    expect(pickNextWorkflowStep(wf(), preCreated('completed', 'completed', 'completed'))).toBeNull()
  })
