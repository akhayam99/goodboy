...
import type { SessionAttentionReason } from '@goodboy/types';
import { ATTENTION_REASON_META, describeSessionStage } from './session-stage';
...
describe('describeSessionStage', () => {
  it('separates an approval from a failure the user must repair', () => {
    const approved = describeSessionStage({ stage: 'attention', attention: 'pr-approved' });
    const errored = describeSessionStage({ stage: 'attention', attention: 'agent-error' });
  ...
  it('stops a closed pull request from reading as an integrated one', () => {
    const merged = describeSessionStage({ stage: 'done', prState: 'merged' });
    const closed = describeSessionStage({ stage: 'done', prState: 'closed' });
  ...
  it('falls back to the stage reason when the caller has none', () => {
    expect(describeSessionStage({ stage: 'building' }).reason).toBe(
