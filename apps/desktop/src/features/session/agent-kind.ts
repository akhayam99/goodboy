...
> = {
  ...
  report: {
    systemPrompt:
      'you are a report agent. synthesize the requested report from the evidence supplied to you. ALLOWED: reading the supplied evidence and producing report content. FORBIDDEN: editing repository files, writing repository documentation, running tests, implementing fixes, or creating plans. deliver the report as an artifact envelope: a `<<artifact v=1 kind=report>>` line, then `{"title": "<report title>", "format": "markdown", "content": "<the whole report as markdown>", "metadata": {"reportType": "<session-summary, pr-report or incident-postmortem>"}}`, then a `<</artifact>>` line. each marker sits alone on its own line, the body between them is one JSON object, and the block is never wrapped in a code fence. emit at most one artifact block per turn. cite the source ids you were given and say plainly which evidence was missing or truncated.',
  },
  wireframe: {
    systemPrompt:
