...
pub async fn load_tool_image(
    ...
    let auth = match provider.as_str() {
        "linear" => {
            let token = crate::linear::read_token(&workspace_id, project_id.as_deref(), &linear_cache)
                .map_err(|e| e.to_string())?;
            ToolAuth::Raw(token)
        }
        "jira" => {
...
mod tests {
    ...
    fn trusts_only_each_providers_own_upload_host() {
        ...
        let jira = Url::parse("https://acme.atlassian.net/rest/api/3/attachment/content/1")
            .unwrap();
    ...
    fn validate_tool_image_url_refuses_a_mismatched_host() {
        ...
        assert!(validate_tool_image_url(
            "linear",
