...
pub fn worktree_changed_files(
    ...
    // Default to origin/main. Non-main-default repos (master, develop, etc.)
    // will need to pass an explicit `base` — that wiring lives outside this
    // function for now; per-workspace base-branch config is a follow-up.
    let user_base = base.unwrap_or_else(|| "origin/main".to_string());
    let resolved = git(p, &["merge-base", "HEAD", &user_base])
        .map(|s| s.trim().to_string())
        .map_err(|_| WorktreeError::Git {
            message: format!("cannot resolve merge-base against {user_base}"),
        })?;
