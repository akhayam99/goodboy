...
**Jira, read and act.** Point Goodboy at a Jira Cloud site and one project key,
...
session from one and the key lands on the branch. From inside Goodboy you can
comment, assign or unassign, move an issue through its own workflow, and edit
the description, no browser tab required. Creating a new Jira issue from
Goodboy is not there yet.
