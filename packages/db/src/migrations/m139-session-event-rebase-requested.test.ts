...
const seedThrough138 = async (): Promise<Database> => {
  const db = await makeMigratedTestDatabase({ throughVersion: 138 });
  await db.execute(
...
const insertEvent = async ({ db, id, kind }: EventParams): Promise<void> => {
  await db.execute(
    ...
    [
      ...
      'session-1',
      kind,
      '{"projectId":"project-1","projectName":"web","branch":"main","behind":126,"agentId":"agent-1"}',
      NOW,
    ],
  );
};

describe('m139 session event rebase requested', () => {
  it('accepts the rebase_requested kind next to the existing kinds', async () => {
    const db = await seedThrough138();

    await migrate(db, migrations);
    await insertEvent({ db, id: 'ev-1', kind: 'rebase_requested' });
    await insertEvent({ db, id: 'ev-2', kind: 'project_materialization_proposed' });
  ...
  it('still refuses a kind nobody declared', async () => {
    ...
    await expect(insertEvent({ db, id: 'ev-bad', kind: 'rebase_completed' })).rejects.toThrow();
  });
});
