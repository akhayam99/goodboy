...
describe('session_external_tasks queries', () => {
  ...
  it('round-trips a bitbucket-provider task', async () => {
    const db = await seed()
    const task = makeTask({
      provider: 'bitbucket',
      externalId: '42',
    ...
    expect(got!.externalId).toBe('42')
    expect(got!.identifier).toBe('acme/web!42')
    expect(got!.url).toBe('https://bitbucket.org/acme/web/pull-requests/42')
  })
