...
export const commitResolveImport = async ({
  ...
  await db.transaction({
    statements: [
      {
        sql: `INSERT INTO resolve_threads (id, session_id, project_id, pr_number, thread_id, origin_kind, state, state_reason, revision, active_attempt_id, disposition, reply_draft, commit_shas_json, question, reply_posted_at, reply_id, github_resolved, closed_at, closed_source, created_at, updated_at)
     ...
     WHERE resolve_threads.revision = 0`,
        params: [JSON.stringify(rows), sessionId, version],
      },
      {
        sql: 'INSERT OR IGNORE INTO resolve_imports (session_id, version, completed_at) VALUES (?, ?, ?)',
        params: [sessionId, version, Date.now()],
      },
    ],
  });
};
