...
describe('workspace_integrations queries', () => {
  ...
  it('round-trips a bitbucket integration with workspaceSlug and host', async () => {
    const db = await seed()
    ...
    const integration: WorkspaceIntegration = {
      id: 'bb-1' as WorkspaceIntegrationId,
      workspaceId,
      provider: 'bitbucket',
      config,
    ...
    const got = await getWorkspaceIntegration(db, workspaceId, 'bitbucket')
    expect(got).not.toBeNull()
    expect(got!.provider).toBe('bitbucket')
    const gotConfig = got!.config as BitbucketIntegrationConfig
    expect(gotConfig.userName).toBe('amin')
    expect(gotConfig.userId).toBe('bb-uuid-1')
    expect(gotConfig.workspaceSlug).toBe('acme')
    expect(gotConfig.host).toBe('https://bitbucket.org')
    expect(got!.credentialKey).toBe(`goodboy.workspace.${workspaceId}.bitbucket`)
  })

  it('bitbucket upsert on conflict preserves id and created_at', async () => {
    const db = await seed()
    const ts1 = new Date('2026-05-21T10:00:00Z').toISOString() as IsoDateTime
    const ts2 = new Date('2026-05-22T10:00:00Z').toISOString() as IsoDateTime
    const initial: WorkspaceIntegration = {
      id: 'bb-orig' as WorkspaceIntegrationId,
      workspaceId,
      provider: 'bitbucket',
      config: {
        userName: 'amin',
    ...
    const updated: WorkspaceIntegration = {
      id: 'bb-new' as WorkspaceIntegrationId,
      workspaceId,
      provider: 'bitbucket',
      config: {
        userName: 'amin2',
    ...
    expect(got!.id).toBe(initial.id)
    expect(got!.createdAt).toBe(initial.createdAt)
    expect((got!.config as BitbucketIntegrationConfig).userName).toBe('amin2')
    expect(got!.credentialKey).toBe('rotated-key')
  })

  it('all four providers coexist for one workspace', async () => {
    const db = await seed()
    const ts = new Date('2026-05-21T10:00:00Z').toISOString() as IsoDateTime
    await upsertWorkspaceIntegration(db, makeIntegration())
    await upsertWorkspaceIntegration(db, makeSentryIntegration())
    await upsertWorkspaceIntegration(
      db,
      makeIntegration({
        id: 'gl-all' as WorkspaceIntegrationId,
        provider: 'gitlab',
        config: { userName: 'a', userId: '1', host: 'https://gitlab.com' },
        credentialKey: `goodboy.workspace.${workspaceId}.gitlab`,
      }),
    )
    await upsertWorkspaceIntegration(db, {
      id: 'bb-all' as WorkspaceIntegrationId,
      workspaceId,
      provider: 'bitbucket',
      config: {
        userName: 'amin',
    ...
    const all = await listIntegrationsForWorkspace(db, workspaceId)
    expect(all.map((i) => i.provider).sort()).toEqual(['bitbucket', 'gitlab', 'linear', 'sentry'])
  })

  it('deleting bitbucket leaves all other providers intact', async () => {
    const db = await seed()
    const ts = new Date('2026-05-21T10:00:00Z').toISOString() as IsoDateTime
    await upsertWorkspaceIntegration(db, makeIntegration())
    await upsertWorkspaceIntegration(db, {
      id: 'bb-del' as WorkspaceIntegrationId,
      workspaceId,
      provider: 'bitbucket',
      config: {
        userName: 'amin',
    ...
    await deleteWorkspaceIntegration(db, workspaceId, 'bitbucket')
    const remaining = await listIntegrationsForWorkspace(db, workspaceId)
    expect(remaining.map((i) => i.provider)).toEqual(['linear'])
    expect(await getWorkspaceIntegration(db, workspaceId, 'bitbucket')).toBeNull()
  })

  it('bitbucket with self-hosted host round-trips host value unchanged', async () => {
    const db = await seed()
    const ts = new Date('2026-06-01T00:00:00Z').toISOString() as IsoDateTime
    await upsertWorkspaceIntegration(db, {
      id: 'bb-self' as WorkspaceIntegrationId,
      workspaceId,
      provider: 'bitbucket',
      config: {
        userName: 'dev',
        userId: 'u9',
        workspaceSlug: 'corp',
        host: 'https://bitbucket.internal.corp.com',
      },
      credentialKey: `goodboy.workspace.${workspaceId}.bitbucket`,
      createdAt: ts,
      updatedAt: ts,
    ...
    const got = await getWorkspaceIntegration(db, workspaceId, 'bitbucket')
    expect((got!.config as BitbucketIntegrationConfig).host).toBe(
      'https://bitbucket.internal.corp.com',
    )
  })
})
