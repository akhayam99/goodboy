...
- last_output_summary: a standard structured document with four fixed sections, in this exact order, each opened by a level-4 markdown heading on its own line: \`#### Problem\`, \`#### Learned\`, \`#### State\`, \`#### Next\`. These four headings are the only markdown headings allowed in any slot value. Every pass MUST emit all four sections, even when one is a single short line.
...
- Do NOT use markdown headings (#), the slot label is already the heading. The only exception is last_output_summary, which MUST open each of its four sections with the mandated \`####\` heading.
