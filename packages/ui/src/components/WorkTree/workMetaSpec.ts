...
export const WORK_META_COLUMN = {
  model:
    'flex w-24 shrink-0 items-center gap-1 overflow-hidden @max-[640px]:w-3 @max-[320px]:hidden',
  modelLabel: 'min-w-0 truncate @max-[640px]:sr-only',
  effort: 'w-13 shrink-0 truncate @max-[760px]:sr-only',
  time: 'w-24 shrink-0 truncate text-right @max-[640px]:w-20 @max-[400px]:hidden',
  cost: 'w-14 shrink-0 truncate text-right @max-[520px]:hidden',
  costRange: 'w-18 shrink-0 truncate text-right @max-[520px]:hidden',
